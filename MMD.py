...
# -*- coding: utf-8 -*-
"""
Created on Sun Aug  9 22:46:16 2020
...
def MMD_Calculate(X,Y,sigma=-1,SelectSigma=2):
...
def MMD_3_Sample_Test(X,Y,Z,sigma=-1,SelectSigma=2,computeMMDs=False):
    ...
    if(sigma<0):
        #Similar heuristics
        if(SelectSigma>1):
            siz=np.min((3000,X.shape[0]))
            sigma1=kernelwidthPair(X[0:siz],Y[0:siz]);
            sigma2=kernelwidthPair(X[0:siz],Z[0:siz]);
            sigma=(sigma1+sigma2)/2.
        else:
            siz=np.min((3000,X.shape[0]*3))
            Zem=np.r_[X[0:siz/3],Y[0:siz/3],Z[0:siz/3]]
            sigma=kernelwidth(Zem);
...
if __name__ == '__main__':
     ...
     X = np.expand_dims(np.random.chisquare(5, 10000),axis = -1)
     Y = np.expand_dims(np.random.chisquare(5,10000),axis = -1)
     Z = np.expand_dims(np.random.normal(0,3,10000),axis = -1)
     
     pvalue,tstat,sigma,MMDXY,MMDXZ = MMD_3_Sample_Test(X,Y,Z,-1,2,True)
